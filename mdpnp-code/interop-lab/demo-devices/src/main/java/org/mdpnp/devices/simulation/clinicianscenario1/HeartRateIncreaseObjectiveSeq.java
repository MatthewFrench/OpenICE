...
/**
 * A sequence of GlobalAlarmSettingsObjective instances.
 */
public final class HeartRateIncreaseObjectiveSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    public HeartRateIncreaseObjectiveSeq() {
        super(HeartRateIncreaseObjective.class);
    }


    public HeartRateIncreaseObjectiveSeq(int initialMaximum) {
        super(HeartRateIncreaseObjective.class, initialMaximum);
    }


    public HeartRateIncreaseObjectiveSeq(Collection elements) {
        super(HeartRateIncreaseObjective.class, elements);
    }
    ...
    public Object copy_from(Object src) {
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    set(i, HeartRateIncreaseObjective.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }
        
        // copy 'new' GlobalAlarmSettingsObjective objects (beyond the original size of this object)
        for(int i = origSize; i < srcSize; i++){
            if (typedSrc.get(i) == null) {
                add(null);
            } else {
                // NOTE: we need to create a new object here to hold the copy
                add(HeartRateIncreaseObjective.create());
